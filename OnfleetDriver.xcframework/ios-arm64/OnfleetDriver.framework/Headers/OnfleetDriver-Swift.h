...
- (void)changeActiveAccountTo:(Account * _Nonnull)account completion:(void (^ _Nullable)(void))completion;
@end
...
/// Main SDK object that provides access to all services.
/// Access shared instance using <code>DriverContext.shared</code>. Before host app starts using <code>shared</code> it
/// must intitate SDK early at application launching sequence. See
/// <code>DriverContext.initSDK(with:environment:app:loggers:)</code>. To check if SDK has been already initialized you can
/// check <code>DriverContext.isInitialized</code> property.
/// DriverContext provides access to following core services:
/// <ol>
///   <li>
///     <em>authentication and accounts</em> -
///     After the SDK is initialized it is a good practice to check if <code>SessionManaging.isAuthenticated</code>
///     returns true. In that case the UI may show main driver/tasks flow, otherwise a login UI should be shown.
///     See <code>SessionManaging</code> for more info.
///   </li>
///   <li>
///     <em>driver and tasks</em> -
///     If driver is successfuly authenticated host app can use <code>DriverManaging</code> protocol to hook up UI
///     to important observable properties such as active task, list of tasks, duty status, etc.
///     see <code>DriverManaging</code>
///   </li>
///   <li>
///     <em>locations</em> -
///     When driver is on duty the SDK tracks user’s location and continously uploads it to the server thus the SDK
///     requires location permissions <em>Always</em> and precise location must be <em>on</em> when on duty.
///     See <code>LocationManaging</code> for more info.
///   </li>
///   <li>
///     <em>attachments uploads</em> -
///     An active task can be supplemented with image attachments. These however don’t block task completion
///     so they are uploaded on background after the task is completed. Host app can observe these events
///     and update UI accordingly. See <code>AttachmentsManaging</code> for more info.
///   </li>
///   <li>
///     <em>data fetching and observing</em> -
...
///     and updated UI accordingly. See <code>DriverContext.refetchData(completion:)</code> and methods
///     in dedicated section below for more info.
///   </li>
///   <li>
...
///     push notification is owned by Onfleet using <code>DriverContext.canHandleRemoteNotification(_:)</code> and if yes it
///     must be forwarded to the SDK using <code>DriverContext.handleRemoteNotification(userInfo:)</code>. Please don’t forget to forward
///     also APN device token using <code>DriverContext.setRemoteNotificationsDeviceToken(_:)</code>
///     See methods in dedicated section below for more info.
///   </li>
/// </ol>
SWIFT_CLASS("_TtC13OnfleetDriver13DriverContext")
@interface DriverContext : NSObject
/// Shared instance of DriverContext.
...
/// Returns true if SDK has been already initialized; false otherwise.
@property (nonatomic, readonly) BOOL isInitialized;
/// Initializes SDK.
/// Even thought <code>DriverContext</code> is a shared instance it has to be initialized once before it’s used
/// first time. Ideal place is somewhere between application will/did finish launching. Subsequential
/// initializing does nothing.
/// \param config A config object that provides information about host application.
///
/// \param environment An environment that will be used. Please note that supported environment
/// is currently <code>Environment.production</code>.
///
/// \param app UIApplication instance
///
/// \param loggers list of loggers that you can pass to SDK for logging. If you don’t pass any loggers
...
/// logger.
///
- (void)initSDKWith:(Config * _Nonnull)config environment:(enum Environment)environment app:(UIApplication * _Nonnull)app loggers:(NSArray<id <LoggerDestination>> * _Nonnull)loggers SWIFT_METHOD_FAMILY(none);
@end

@class NSData;

@interface DriverContext (SWIFT_EXTENSION(OnfleetDriver))
/// Pass device token once obtained from app delegate
- (void)setRemoteNotificationsDeviceToken:(NSData * _Nonnull)data;
/// This can be used to determine if the notification will be handeled by SDK.
/// When a push notification is received you should use this method to determine wether it should be
/// passed to SDK via <code>DriverContext.handleRemoteNotification(userInfo:)</code> or not.
///
/// returns:
/// false if SDK does not handle the push notification’s <code>userInfo</code>.
- (BOOL)canHandleRemoteNotification:(NSDictionary * _Nonnull)userInfo SWIFT_WARN_UNUSED_RESULT;
/// Handle remote notification passed from app delegate.
/// You can verify if SDK is capable of handling a push notification using
/// <code>DriverContext.canHandleRemoteNotification(_:)</code>. If that method returns true you
/// don’t have to pass push notification to SDK.
/// note:
...
@interface DriverContext (SWIFT_EXTENSION(OnfleetDriver))
/// Returns true if data is stale. This can happen if there is no connection to remote server for some time.
@property (nonatomic, readonly) BOOL isDataStale;
@end


@interface DriverContext (SWIFT_EXTENSION(OnfleetDriver))
/// Getter for <code>deviceId</code> that is generated by SDK.
...
@interface DriverContext (SWIFT_EXTENSION(OnfleetDriver))
/// :nodoc:
- (void)userInterfaceDidBecomeReady;
@end
...
/// Provides reason why log out operation was performed.
/// Logout can happen for main reasons:
/// <ol>
///   <li>
///     explicit logout - performed by user by tapping on logout button in UI
///   </li>
///   <li>
///     implicit logout - performed by Onfleet platform due to various reasons.
///   </li>
/// </ol>
typedef SWIFT_ENUM(NSInteger, LogoutReason, open) {
/// Initiated by user in UI
  LogoutReasonUserInitiated = 0,
/// Initiated by user in UI using force flag
  LogoutReasonUserInitiatedByForce = 1,
/// Initiated when user rejects an account invitation and there are no other accounts to set as active.
  LogoutReasonRejectedAccountInvitation = 2,
/// Initiated by platform due to un-recoverable authentication state
  LogoutReasonInvalidAuthentication = 3,
/// Initiated by platform due to no driver account found
  LogoutReasonNoDriver = 4,
/// Initiated by platform due to no organization found
  LogoutReasonNoOrganization = 5,
/// Initiated by platform due driver account removed from an organization and no other accounts to set
/// as active.
  LogoutReasonAccountRemoved = 6,
/// Initiated by platform due to suspended organization
  LogoutReasonOrganizationSuspended = 7,
/// Initiated by platform due activity on other device. Driver can be authenticated at the same time on
/// only 1 device.
